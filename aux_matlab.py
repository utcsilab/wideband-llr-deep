...
def decode_matlab_file(eng, code_type, input_llr, ref_bits, num_snr,
                       num_codewords, mode='soft'):
    # Static path
    decoder_path = '/matlab/CoreLDPCDecoderFile.m'
    ...
    # Convert to arrays
    rec_bits = np.double(np.asarray(rec_bits))
...
# Train an MI quantizer with an unrolled LLR vector by calling MATLAB
def train_mi_quantizer(eng, input_llr, mod_size, num_bits,
                       num_tx, num_rx,
                       train=True):
    # Static paths
    trainer_path = '/matlab/designQuantizerFile.m'
    
    # Draw a random integer (independent of global seed)
    rg         = Generator(PCG64())
    random_idx = np.round(1e10 * rg.standard_normal()).astype(np.int)
    # Filenames
    filename_in  = '/matlab/scratch/in%d.mat' % random_idx
    filename_out = '/matlab/sota_mimo%dby%d_qam%d_bits%d.mat' % (
